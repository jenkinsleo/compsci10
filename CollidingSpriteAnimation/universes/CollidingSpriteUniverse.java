...
public class CollidingSpriteUniverse implements Universe {
	...
	private ExcitableBackground background = new ExcitableBackground();	
	...
	public CollidingSpriteUniverse () {
		...
		for (int row = 0; row < ROWS; row++) {
			for (int col = 0; col < COLS; col++) {
				...
				if (row == 0 && col == 0) {
					player1 = new LDNJSprite(minX + COL_WIDTH / 2 + 8, minY + ROW_HEIGHT / 2 + 8);
					sprites.add(player1);					
				}
					...
					sprites.add(new SimpleSprite(minX + COL_WIDTH / 2 + 8, minY + ROW_HEIGHT / 2 + 8));
				}
				else {
					//place a coin?
